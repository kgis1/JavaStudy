...
class NormalClass
{
	...
	void classMethod() {
		for (int i=1 ; i<=10 ; i++) {
			System.out.printf("%si=%d%n", title, i);
		}
	}
}
/*
독립쓰레드(Non daemon thread)
: 메인스레드와 Working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로
...
class NormalThread extends Thread
{
	...
	public NormalThread() {}
	/*
	Thread클래스 생성자 호출시 전달하는 인수를 통해 이 쓰레드의
	...
	발생할 수 있으므로, 예외처리를 해야한다. 여기서는 try~caatch로
	직접 처리하지 않고 throws해서 호출한 위초 예외던지기를 하고
	있다. */
...
class DaemonThread extends Thread
{
	...
	public void run() {
		/*
		무한루프로 구성되어있고 별도의 탈줄조건을 만들지 않는다.
...
public class Ex03DaemonThread {
	...
	{	
		...
		실행되어 1번 인스턴스가 작업을 맞쳐야만 2번째가 실행된다. */
		...
		는 방법으로 아래 2가지가 있다. */
		//1ㅏㅇ법1: 생성자를 통해 쓰래드의 이름 부여
		NormalThread yt1 = new NormalThread("1st 쓰레드");
		//방법2: setName()을 통해 아름 부여
		//yt1.setName("First 쓰레드");
		yt1.start();
		/*
		두번째 독립쓰레드 생성 및 시작. 쓰레드명을부여하지 않았으
		...
		yt2.start();
		/*
		데몬쓰레드는 독입쓰레드를 먼저 생성한 후 setDaemon을 통해
		지정할 수 있다.
		 */
